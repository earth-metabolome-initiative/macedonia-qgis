...
def test_resolve_bbox_defaults_to_macedonia_extent() -> None:
...
def test_country_name_from_code_accepts_alpha2_and_alpha3() -> None:
    assert country_name_from_code("MK") == "North Macedonia"
    assert country_name_from_code("mkd") == "North Macedonia"
...
def test_resolve_country_place_id_uses_inaturalist_country_place() -> None:
    def fake_lookup_places(**kwargs: object) -> dict[str, object]:
        assert kwargs["q"] == "North Macedonia"
        return {
            "results": [
                {"id": 123, "name": "Some Macedonia Place", "admin_level": None},
                {
                    "id": 147425,
    ...
    assert resolve_country_place_id("MK", lookup_places=fake_lookup_places) == (
        147425,
        "North Macedonia",
    )
